...
Sometimes, you'll have to manually update the Apollo cache after a mutation. Apollo will try to update the cache automatically if it has all the information it needs. For example, if you send a mutation that updates the title of a tag, and the mutation returns that tag with both its `nodeId` and new `title` in the response, Apollo will automatically update all instances of that tag in the cache with the new title. However if you create a new tag, Apollo might not automatically understand that it tags to add that need to various lists in the cache. That's when you need to manually update the cache. Read more about that in the Apollo docs, or look at the `CreateTag` component for an example. 
...
# Documentation that came with Create React App (MAY BE DEPRECATED!)

**Some of the following information will still hold true, but we have ejected from Create React App and stared modifying things. We have tried to remove pieces of the following documentation that we know are outdated, but we may have missed something. It may still be useful if you're debugging. Use as inspiration and not a definite source of truth.**
